...
class AccidentsGUI:
    def __init__(self, master: tk.Tk):
        ...
        ttk.Separator(frm).grid(column=0, row=4, columnspan=6, sticky="ew", pady=8)
        ttk.Button(frm, text="Show Monthly Chart", command=self.show_monthly_chart, style="Accent.TButton").grid(column=0, row=5, sticky="w")
        ttk.Button(frm, text="Show Top Months (Top 5)", command=self.show_top_months_chart, style="Accent.TButton").grid(column=1, row=5)
        ttk.Button(frm, text="Show Top Vehicles (Top 5)", command=self.show_top_vehicles_chart, style="Accent.TButton").grid(column=2, row=5)
        ttk.Button(frm, text="Show Top Streets Chart", command=self.show_top_streets_chart, style="Accent.TButton").grid(column=3, row=5)
        ttk.Button(frm, text="Show Heatmap", command=self.show_heatmap, style="Accent.TButton").grid(column=4, row=5)
    ...
    def filter_and_show(self):
        ...
        def worker():
            ...
            def done():
                ...
                # Basic numeric stats
                self.output.insert(tk.END, f"Total accidents: {stats.get('total_accidents', 0)}\n", "muted")
                self.output.insert(tk.END, f"Injured: {stats.get('number_of_persons_injured', 0)}\n", "muted")
                self.output.insert(tk.END, f"Killed: {stats.get('number_of_persons_killed', 0)}\n", "muted")

                # Top months
                top_months = stats.get('top_months', {})
                if top_months:
                    self.output.insert(tk.END, "\nTop months (by accidents):\n", "header")
                    for m, c in top_months.items():
                ...
                if top_vehicles:
                    ...
                    for v, c in top_vehicles.items():
                        self.output.insert(tk.END, f"  {v}: {c}\n", "muted")

                # Top streets (truncate long lists)
                top_streets = stats.get('top_streets', {})
                if top_streets:
                    self.output.insert(tk.END, "\nTop streets:\n", "header")
                    for s, c in list(top_streets.items())[:10]:
                        self.output.insert(tk.END, f"  {s}: {c}\n", "muted")
    ...
    def show_top_months_chart(self):
        if self.filtered is None:
            messagebox.showwarning("No data", "Load and filter data first")
            return
        try:
    ...
    def show_top_vehicles_chart(self):
        if self.filtered is None:
            messagebox.showwarning("No data", "Load and filter data first")
            return
        try:
